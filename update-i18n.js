...
const dir = 'd:/AI/digital-marketplace';
...
const switcherHtml = `<div class="flex items-center gap-2 border-r border-[var(--border)] pr-3 mr-1 rtl:border-r-0 rtl:border-l rtl:pr-0 rtl:pl-3 rtl:mr-0 rtl:ml-1">
...
files.forEach(file => {
  ...
  if (!content.includes('i18n.js')) {
    content = content.replace('<script src="js/main.js"></script>', '<script src="js/i18n.js"></script>\n<script src="js/main.js"></script>');
  }
  
  if (!content.includes('data-lang="en"')) {
    // There are some pages where button class="dark-toggle p-2..." and others "dark-toggle icon-btn"
    content = content.replace(/<button class="dark-toggle/g, switcherHtml + '\n<button class="dark-toggle');
  }

  for (const [text, replacement] of Object.entries(i18nMapping)) {
